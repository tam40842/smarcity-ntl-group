      ...
      <b-tab>
        ...
        >
          ...
          <b-dropdown
            ...
            v-if="titleOptions && titleOptions.length > 0"
          >
            <template v-for="(title, key) in titleOptions">
              <b-dropdown-item @click="changeTitle(key)" :key="key" href="#"
                >{{ title.toUpperCase() }}
              </b-dropdown-item>
            </template>
          </b-dropdown>
        ...
        <div class="box-content mt-2">
          <b-row v-if="typeForm && typeForm.length > 0">
            <template v-for="(value, i) in typeForm">
              ...
              <template
                v-else-if="value.type && value.type.toUpperCase() == 'HIDE'"
              >
                <b-colxx :key="i" :sm="value.col"></b-colxx>
              </template>
              <template
                v-else-if="value.type && value.type.toUpperCase() == 'DATETIME'"
              >
                <b-colxx :key="i" :sm="value.col">
                  <b-form-group :label="value.label">
                    <div class="d-flex">
              ...
              <template
                v-else-if="value.type && value.type.toUpperCase() == 'NUMBER'"
              >
                <b-colxx :key="i" :sm="value.col">
                  <b-form-group :label="value.label">
                    <b-form-input
                ...
                <b-colxx :key="i" :sm="value.col">
                  <b-form-group :label="value.label">
                    <b-form-input
                      ...
                      v-model="dataForm[value.key]"
                      :disabled="
                        (isEdit || isAdd) && value.type != 'key' ? false : true
                      "
                    ></b-form-input>
                  </b-form-group>
                </b-colxx>
...
export default {
  ...
  data() {
    return {
      ...
      keyComponent: "basic",
      //title-options
      currentTitle: this.title ?? "NGHIỆP VỤ",
      //table-chil-by-key
      fieldsByKey: {},
      dataByKey: {},
      //search
  ...
  methods: {
    ...
    changeTitle(index) {
      if (this.titleOptions?.length > 0) {
        this.currentTitle = this.titleOptions[index];
      }
    },
    ...
    updateTab(value) {
      ...
      if (value == 1) {
        this.currentTitle = this.title ?? "NGHIỆP VỤ";
      }
      if (value == 2) {

  ...
  <div style="background: white;position: relative">
    ...
    <div>
      <canvas
        class="canvas"
        :id="
          `canvasfull-${dataCameraFull.Channel}-${'' +
            dataCameraFull.LocationID}`
        "
        style=" width: 100%"
      ></canvas>
    </div>
...
export default {
  ...
  async mounted() {
    ...
    if (this.dataCameraFull.DeviceStatus) {
  ...
  methods: {
      ...
      fetch(
        ...
        .then((data) => {
          if (data && data.success && data.status === 'START') {
            ...
            this.playWS(
              data.SID,
              this.dataCameraFull.Channel,
              '' + this.dataCameraFull.LocationID,
            )
          }
        })
      this.loading = false
    ...
    playWS(port, channel, location) {
      ...
      new JSMpeg.Player(`${this.serviceSocket}:${port}`, {
        canvas: document.getElementById(`canvasfull-${channel}-${location}`),

        ...
        <b-col md="12">
          <div class="icon-cards-row">
            <glide-component :settings="glideIconsOption">
              <template
                v-for="(slide, index) in dataLayer.dataList"
                v-if="dataLayer.dataList"
              >
...
export default {
  ...
  methods: {
    flyToPoint(item) {
      this.$refs.formMapTree.flyToPoint(item.StationLat, item.StationLong);
    },
    SetDataChart(data) {
      let textBar = "";
      ...
      if (data && data.length > 0) {
        for (let i = 0; i < data.length; i++) {
          ...
          if (
            data[i].IndicatorCode === "TMPSL" ||
            data[i].IndicatorCode === "PH"
          ) {
            if (textMixed === "") {

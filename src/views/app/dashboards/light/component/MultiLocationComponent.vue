...
<template>
  <div style="height: 100%">
    ...
    <b-modal
      :id="`modal-detail-mul-light${dataForm.CabinetID}`"
      ref="modal-detail"
      hide-header
      v-if="dataForm"
...
export default {
  ...
  methods: {
    ...
    async showModal(item) {
      ...
      if (this.dataForm) {
        setTimeout(() => {
          this.$bvModal.show(`modal-detail-mul-light${item.CabinetID}`);
        }, this.timer);
      }
    },
    initMap() {
      ...
      if (location.length > 0) {
        ...
        for (let i = 0; i < location.length; i++) {
          ...
          if (location[i].length > 2) {
            ...
            if (this.dataMapNow[i].StationImage) {
              urlImage = this.dataMapNow[i].StationImage;
            }
            ...
            contentPopup =
              ...
              this.dataMapNow[i].StationName +
              "</strong></p>" +
              '<p class="pl-3 pr-3 font-italic" style="font-size: 12px; margin-top: -10px; line-height: 1.3">' +
              this.dataMapNow[i].StationAddress +
              "</p>" +
              '<p class="pl-3 pr-3 font-italic" style="font-size: 12px; margin-top: -10px; line-height: 1.3">' +
              this.$t("map.update-time") +
              ": " +
              '<span class="text-danger font-weight-bold">' +
              handling.convertDateTime(this.dataMapNow[i].UpdateTime) +
              "</span></p>" +
              "</div>";
            ("</div>");
            ...
            marker.on("click", () => {
              this.showModal(this.dataMapNow[i]);
            });
          } else {
            if (this.typeItem) {
              icon = new L.Icon({

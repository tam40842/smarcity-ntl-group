...
export default {
  ...
  props: ['latLng', 'typeItem', 'convertItems', 'dataMapNow', 'objectData',],
  data() {
    return {
      keyString: null,
      map: null,
  ...
  methods: {
    ...
    async showModal(item) {
      this.$emit('showModalDetail', item)
      // if (this.dataMapNow && this.dataMapNow.length > 0) {
      //   this.dataForm = this.dataMapNow.find((data) => data.StationID == item.StationID)
      //   console.log(this.dataForm, 'showModal')
      // }
      // if (this.dataForm) {
      //   setTimeout(() => {
      //     this.$bvModal.show('led-station-detail-modal')
      //   }, this.timer)
      // }
    },
    initMap() {
      ...
      if (location.length > 0) {
        ...
        for (let i = 0; i < location.length; i++) {
          ...
          if (location[i].length > 2) {
            ...
            console.log(this.convertItems);
            console.log(this.convertItems[i]);
            if (this.convertItems[i].LinkAvatar) {
              urlImage = this.convertItems[i].LinkAvatar
            }
            contentPopup =
              ...
              this.convertItems[i].LocationName +
              '</strong></p>' +
              ...
              handling.convertDateTime(this.convertItems[i].UpdateTimeNoFormat) +
              '</span></p>' +
              '</div>'
            ;('</div>')
    ...
    cancelForm() {
      ...
      this.$bvModal.hide('led-station-detail-modal')
    }
  },
}
</script>

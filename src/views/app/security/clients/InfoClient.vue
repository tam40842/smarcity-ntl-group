  ...
  <div v-if="objectData">
    ...
    >
      <div class="text-center">
        <i
          style="color: #cf091c; font-size: 24px"
          class="fad fa-exclamation-triangle"
        ></i>
      </div>
      <div class="text-center font-weight-bold">
        {{ $t("notify.noti-form-customers") }}
        <p class="font-italic font-weight-bold">
          {{ $t("notify.noti-form-customers-2") }}
        </p>
      </div>
    </b-modal>
  </div>
...
export default {
  ...
  methods: {
    addModal() {
      ...
      this.typeForm = "ADD";
      this.apiForm = "/api/Customers/Add";
    ...
    editModal(items) {
      if (items[0].GroupID == 9999) {
        this.$bvModal.show("notify-modal");
      } else {
        this.getDataByID(items[0].CusID);
        this.showDataFormOptionParentCustomerGroup(this.optionGroup);
        setTimeout(() => {
          ...
          this.typeForm = "EDIT";
          this.apiForm = "/api/Customers/Edit";
          this.bodyFormDefault = {
            CusID: items[0].CusID,
            MenuIDCurent: this.objectData.menuIDCurrent,
          };
    ...
    deleteModal(items) {
      this.$bvModal
        .msgBoxConfirm(this.$t("form.question") + items[0].CusFullName + "?", {
        ...
        .then((value) => {
          if (value === true) {
            this.handleSubmitDelete(items[0].CusID);
          }
        })
        .catch((err) => {
          console.log(err);
    ...
    //     MenuIDCurent: this.objectData.menuIDCurrent
    //   }
    //   employerResetPassword(body).then(val => {
    //     if (val.status) {
    //       this.getData()
    //       setTimeout(() => {
    ...
    showDataFormOptionParentCustomerGroup(array, id) {
      ...
      this.dataFormOptions.GroupID = newArray;
    },
    getOptionGroup() {
      groupAPI
  ...
  async created() {
    ...
    await this.getColumn("Customers");
    await this.getKeyTable();
    await this.getData();
    await this.getFormAddShow("Customers");

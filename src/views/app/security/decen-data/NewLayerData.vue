        ...
        <b-row class="mb-4">
          <template v-if="items.length > 0">
            <template v-for="(item, index) in items">
              <b-colxx cols="12" class="" :key="index" :id="item.StationID">
                <b-card :class="{ 'card-option-camera d-flex flex-row': true }">
                  <div 
                    class="pl-2 d-flex flex-grow-1 min-width-zero click-select"
                    :class="currentGeoCode && currentGeoCode.StationID == item.StationID && 'selected'"
                    @click="changeGeoCode(item)"
                  >
                    <div
                    ...
                    <div
                      class="custom-control custom-checkbox pl-2 align-self-center"
                    >
                      <b-form-checkbox
      ...
      systemAPI
        ...
        .then((val) => {
          ...
          if(this.items.length) {
            this.$emit('changeGeoCode', this.items[0])
          }
        })
        .catch((err) => {
          console.log(err);

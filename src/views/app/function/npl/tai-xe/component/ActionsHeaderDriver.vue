<template>
  <div class="action-header-tree">
    ...
    >
      <b
        >{{ currentTotalIndex > 0 ? currentIndex + 1 : currentIndex }} /
        {{ currentTotalIndex }}</b
      >
    </b-button>
    <b-button
    ...
    <b-button
      v-if="showPrint"
      :disabled="disablePrint || currentTotalIndex == 0"
    ...
    <b-button
      v-if="showFind"
      :disabled="disableFind || currentTotalIndex == 0"
...
export default {
  ...
  setup(_, { emit }) {
    ...
    const emitChangeActions = (bol, key) => {
      emit("change-actions", bol, key);
    };

<template>
  <div class="main">
    ...
    <b-button squared variant="outline-secondary" size="xs">
      <b>{{ items.length == 0 || !selectedItem ? 0 : index + 1 }}/{{ items.length }}</b>
    </b-button>
    <b-button
      squared
      variant="outline-secondary"
    ...
    <b-button
      ...
      variant="outline-primary"
      size="xs"
      :disabled="isReadonly == false ? true : false"
      @click="emitChangeActions('COPY')"
    >
      <i class="fas fa-copy" />
      <b>Sao chép</b>
    </b-button>
    ...
    <b-button
      ...
      variant="outline-primary"
      size="xs"
      :disabled="(selectedItem && selectedItem.IsActive.value == 0)
          ? false
          : true
      "

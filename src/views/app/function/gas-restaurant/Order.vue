<template>
  <div v-if="objectData">
              ...
              <b-dropdown-item
                @click="
                  selectedItems[0].StatusID > 0
                    ? fullInfoItem(selectedItems)
                    : infoModal(selectedItems)
                "
                v-if="
    ...
    <custom-form
      ...
      :hide-footer="this.dataForm.StatusEdit === 0 ? true : false"
      v-if="nameForm"
    ></custom-form>
    <full-order-form
...
import CustomForm from "./component/OrderForm";
    ...
    infoModal(items) {
      ...
      setTimeout(() => {
        this.dataForm = {
          ...
          StatusEdit: this.dataByID.StatusEdit,
        ...
        this.nameForm = "modal-approval";
        this.titleForm = this.$t("form.title-info").toUpperCase();
        this.typeForm = "INFO";
        this.apiForm = "/api/GasContracts/Edit_Approval";
    ...
    getDataByID(id) {
      ...
      gasRestaurantAPI
        ...
        .then((val) => {
          ...
          if (this.dataByID) {
            this.getOptionSaler(this.dataByID.StoreID);
          }
        })
        .catch((err) => {
          console.log(err);

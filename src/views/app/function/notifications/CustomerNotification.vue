...
export default {
  ...
  methods: {
    ...
    addModal() {
      ...
      this.typeForm = "ADD";
      this.apiForm = "/api/CustomerNotifications/Add";
    ...
    handleSubmit(val) {
      if (val.status) {
        if (!this.items || this.items.length === 0) {
          ...
          this.getColumn("CustomerNotifications");
          this.getKeyTable();
    ...
    editModal(items) {
      ...
      setTimeout(() => {
        ...
        this.typeForm = "EDIT";
        this.apiForm = "/api/CustomerNotifications/Edit";
        this.bodyFormDefault = {
          ID: items[0].ID,
          MenuIDCurent: this.objectData.menuIDCurrent,
        };
    ...
    async handleSubmitDelete(id) {
      ...
      notificationAPI
        .removeCustomerNotifications(body)
        .then((val) => {
          if (val.status) {
    ...
    statusIsSend(data) {
      ...
      notificationAPI
        .resetSendCustomerNotifications(body)
        .then((val) => {
          if (val.status) {
            setTimeout(() => {
              this.getData(this.fromDateDefault, this.toDateDefault);
    ...
    statusEdit(data) {
      ...
      notificationAPI
        .changeStatusCustomerNotifications(body)
        .then((val) => {
          if (val.status) {
            setTimeout(() => {
              this.getData(this.fromDateDefault, this.toDateDefault);
    ...
    getDataByID(id) {
      ...
      notificationAPI
        .getCustomerNotificationsByID(body)
        .then((val) => {
    ...
    getOptionGroup() {
      groupAPI
        .recursiveGroupCustomersActiveAll()
        .then((val) => {
    ...
    showDataFormOptionParentCustomerGroup(array, id) {
      ...
      newArray[0] = { ...newArray[0], keyString: "all" };
      this.dataFormOptions.DataKey = newArray;
    },
  },
  async created() {
    await this.getListMenuRight();
    await this.getColumn("CustomerNotifications");
    await this.getKeyTable();
    await this.getData(this.fromDateDefault, this.toDateDefault);
    await this.getFormAddShow("CustomerNotifications");

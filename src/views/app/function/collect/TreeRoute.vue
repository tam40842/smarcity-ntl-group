...
export default {
  ...
  methods: {
    ...
    editModal(items) {
      ...
      this.showDataFormOptionRoute(this.items, items[0].TreeRouteID);
      setTimeout(() => {
        this.dataForm = handling.showExtensionFormEdit(this.dataByID);
        for (let i = 0; i < this.items.length; i++) {
          if (this.items[i].TreeRouteID === items[0].TreeRouteID) {
    ...
    deleteModal(items) {
      this.$bvModal
        .msgBoxConfirm(
          this.$t("form.question") + items[0].TreeRouteName + "?",
          {

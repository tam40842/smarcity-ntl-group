          ...
          <b-col lg="4" md="6" class="text-left pt-2">
            <b class="list-item-heading mb-1 truncate text-small">
              ...
              <span v-if="data.IsClose" class="text-danger">
                ({{ $t("modal.device-close").toUpperCase() }})
              </span>
              <span v-else class="text-success">
                ({{ $t("modal.processing").toUpperCase() }})
              </span>
            </b>
          </b-col>
        ...
        <b-row>
          <b-col md="12">
            <b-progress
              ...
              height="30px"
            >
              <template v-for="(item, index) in data.Employers">
                <b-progress-bar
                  :key="index"
            ...
            <div class="text-center mt-3">
              <template v-if="data.Employers && data.Employers.length > 0">
                <template v-for="(item, index) in data.Employers">
                  <strong :key="index" class="mr-1">
                    <i :style="'color:' + item.color" class="fas fa-circle"></i>
                    {{ item.FullName }} ({{ item.PercentComptete }}%)
                  </strong>
                </template>
              </template>
              <template v-else>
...
export default {
  ...
  computed: {
    percentCompteteTotal() {
      let result;
      if (this.data && this.data.Employers) {
        this.data.Employers.forEach((ite) => {

      ...
      <b-row>
        ...
        <b-colxx md="12">
          <b-row>
            ...
            <b-colxx md="12">
              <b-form-checkbox
                v-model="statusWarning"
                class="custom-checkbox"
                :value="true"
                :unchecked-value="false"
              >
                {{ $t("table.temperature-warning") }}
              </b-form-checkbox>
...
export default {
  ...
  watch: {
    ...
    items() {
      this.getColumn("CameraThermals");
      this.getKeyTable();
    },
    statusWarning(to, from) {
      if (this.dataModal) {
        if (to) {
          this.hisList = this.hisList.filter((x) => x.IsWarning === 1);
        } else {
  ...
  methods: {
    ...
    //   }
    //   //api-setCallback edit
    //   thermalAPI.thermalCameraReset(body)
    //     .then((val) => {
    //       if (val.status) {
    //         this.makeToast(
    //           'success',
    ...
    resetSystemModal(items) {
      this.$bvModal
        .msgBoxConfirm(
          this.$t("form.question-reset") + items[0].CameraThermalName + "?",
          {
  ...
  async created() {
    ...
    await this.getColumn("CameraThermals");

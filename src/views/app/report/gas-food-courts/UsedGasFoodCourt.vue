...
export default {
  ...
  data() {
    return {
      ...
      exportStatus: "close",
      typeFile: "Report-Used-Total",
      titleForm: null,
      fromE: moment().format("YYYY-MM-DD"),
      toE: moment().format("YYYY-MM-DD"),
      selectedOption: "Report-Used-Total",
      options: [
        {
          text: "Tổng hợp",
          value: "Report-Used-Total",
        },
        {
          text: "Chi tiết",
          value: "Report-Used-Details",
        },
      ],
    };
  },
  watch: {
    items() {
      this.getColumn("ReportByGasFoodCourts");
    },
  },
  methods: {
    resetModal() {
    ...
    convertKeyTableData(array, ParentID, arrayTable = [], i) {
      array.forEach((item) => {
        ...
        let obj = {
          ...
          Level: item["Level"],
          SortID: item["SortID"],
          CusTypeAddress: item["CusTypeAddress"],
          CountValue: item["CountValue"],
          DetectedValues: item["DetectedValues"],
          GasDectectorName: item["GasDectectorName"],
          GasDetectID: item["GasDetectID"],
          LevelNameName: item["LevelNameName"],
          UsedValues: item["UsedValues"],
          UpdateTime: item["UpdateTime"],
          TotalDay: item["TotalDay"],
          BalanceQtty: item["BalanceQtty"],
          TotalQtty: item["TotalQtty"],
          DebitQtty: item["DebitQtty"],
          Key: i,
        };
        arrayTable.push(obj);
        this.convertKeyTableData(array, item["CusTypeID"], arrayTable, i + 1);
      });
    },
  ...
  async created() {
    ...
    await this.getColumn("ReportByGasFoodCourts");
  },
  computed: {
    fields: function () {

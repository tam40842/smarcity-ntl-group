...
import LampErrorTotalDetail from "./details/LampErrorTotalDetail";

export default {
  components: {
    "custom-table": CustomTables,
    "custom-form": CustomForm,
    "lamp-error-total-detail": LampErrorTotalDetail,
  },
  data() {
    return {
      ...
      dataForm: null,
      isModalInfo: false,
      keyRowClass: "WATERWAYLAMPERROR",
    };
  },
  watch: {
    items() {
      this.getColumn("WaterwayLampErrorReports");
    },
  },
  methods: {
    infoModal(items) {
    ...
    rowSelected(data) {
      ...
      if (this.selectedItems.length == 0) {
        ...
        this.selectedItems[0].IsDetail == 1
  ...
  async created() {
    await this.getKeyTable(this.from, this.to);
    await this.getData(this.from, this.to);
    await this.getListMenuRight();
    await this.getColumn("WaterwayLampErrorReports");
  },
  computed: {
    fields: function () {

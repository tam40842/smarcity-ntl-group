    ...
    <b-row>
      <b-colxx xxs="12" class="mb-4">
        <b-card>
          <b-row>
            <b-colxx lg="4">
              <b-form-group :label="$t('modal.employer-option') + ':'">
                <b-form-select
                  v-model="selectedEmployer"
                  :options="optionEmployer"
                  size="sm"
                  @change="changeOption(selectedEmployer, from, to)"
                ></b-form-select>
              </b-form-group>
            </b-colxx>
            <b-colxx lg="4">
              <b-form-group :label="$t('modal.from-date') + ':'">
                <b-form-datepicker
                  ...
                  v-model="from"
                  required
                  @input="changeOption(selectedEmployer, from, to)"
                ></b-form-datepicker>
              </b-form-group>
            </b-colxx>
            <b-colxx lg="4">
      ...
      artificialIntelligenceAPI
        ...
        .then((val) => {
          ...
          if (result.length > 0) {
            ...
            this.optionEmployer = array;
            this.selectedEmployer = this.optionEmployer[0].value;
            setTimeout(() => {
              this.getData(this.selectedEmployer, this.from, this.to);
              this.getKeyTable(this.selectedEmployer, this.from, this.to);
            }, this.timer);
          }
        })
        .catch((err) => {

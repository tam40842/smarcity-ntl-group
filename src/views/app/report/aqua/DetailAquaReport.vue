        ...
        <div class="top-right-button-container">
          <b-button-group>
            <b-dropdown right :text="$t('dropdown.action')" variant="primary">
              <b-dropdown-item
                @click="printExternalExcel('detail-aqua-report')"
              >
  ...
  methods: {
    ...
    getUnit(selected) {
      ...
      aquaAPI
        .listUnit(body)
        .then((val) => {
          let result = val.status ? val.data.Table : [];
          let array = [];
          if (result.length > 0) {
            result.forEach((_val) => {
              let obj = {
    ...
    getDataTable(id, unit, from, to) {
      ...
      aquaAPI
        ...
        .then((val) => {
          this.data = val.status ? val.data.Table : [];
        })
        .catch((err) => {
          console.log(err);
        });

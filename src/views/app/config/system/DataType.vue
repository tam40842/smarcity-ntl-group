...
<script>
import CustomTables from "./component/CustomTables.vue";
...
export default {
  ...
  methods: {
    ...
    editModal(items) {
      this.getDataTypeByID(items[0].DataTypeID);
      setTimeout(() => {
    ...
    deleteModal(items) {
      this.$bvModal
        .msgBoxConfirm(this.$t("form.question") + items[0].DataTypeName + "?", {
    ...
    async handleSubmitDelete(id) {
      ...
      systemAPI.dataTypeRemove(body).then((val) => {
        if (val.status) {

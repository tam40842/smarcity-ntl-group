...
export default {
  ...
  methods: {
    ...
    editModal(items) {
      this.getDataByID(items[0].GasUnitsID);
      setTimeout(() => {
        ...
        this.typeForm = "EDIT";
        this.apiForm = "/api/GasUnits/Edit";
        this.bodyFormDefault = {
          ID: items[0].GasUnitsID,
          MenuIDCurent: this.objectData.menuIDCurrent,
        };
        setTimeout(() => {
          this.$bvModal.show(this.nameForm);
    ...
    deleteModal(items) {
      this.$bvModal
        .msgBoxConfirm(this.$t("form.question") + items[0].GasUnitsName + "?", {
        ...
        .then((value) => {
          if (value === true) {
            this.handleSubmitDelete(items[0].GasUnitsID);
          }
        })
        .catch((err) => {
          console.log(err);
  ...
  async created() {
    ...
    await this.getColumn("GasUnits");
    await this.getKeyTable();
    await this.getData();
    await this.getFormAddShow("GasUnits");
  },
  watch: {
    items() {
      this.getColumn("GasUnits");

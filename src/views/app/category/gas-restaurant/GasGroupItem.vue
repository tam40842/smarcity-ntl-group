...
export default {
  ...
  methods: {
    ...
    addModal() {
      ...
      this.typeForm = "ADD";
      this.apiForm = "/api/GasItemGroups/Add";
    ...
    editModal(items) {
      this.getDataByID(items[0].GasItemGroupsID);
      setTimeout(() => {
        ...
        this.typeForm = "EDIT";
        this.apiForm = "/api/GasItemGroups/Edit";
        this.bodyFormDefault = {
          ID: items[0].GasItemGroupsID,
          MenuIDCurent: this.objectData.menuIDCurrent,
        };
        setTimeout(() => {
          this.$bvModal.show(this.nameForm);
    ...
    deleteModal(items) {
      this.$bvModal
        .msgBoxConfirm(
          this.$t("form.question") + items[0].GasItemGroupsName + "?",
          {
        ...
        .then((value) => {
          if (value === true) {
            this.handleSubmitDelete(items[0].GasItemGroupsID);
          }
        })
        .catch((err) => {
          console.log(err);
  ...
  async created() {
    ...
    await this.getColumn("GasItemGroups");
    await this.getKeyTable();
    await this.getData();
    await this.getFormAddShow("GasItemGroups");
  },
  watch: {
    items() {
      this.getColumn("GasItemGroups");

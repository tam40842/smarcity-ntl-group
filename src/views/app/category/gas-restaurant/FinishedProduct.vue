...
export default {
  ...
  methods: {
    ...
    addModal() {
      this.apiForm = "/api/StationByRestaurents/ExportProduct";
      setTimeout(() => {
        ...
        .then((val) => {
          if (val.status) {
            if (!this.items || this.items.length === 0) {
              ...
              this.getColumn("StationByGasRestaurants");
              this.getKeyTable();
              this.getData();
              setTimeout(() => {
                this.makeToast(
    ...
    editModal(items) {
      ...
      setTimeout(() => {
        ...
        this.typeForm = "EDIT";
        this.apiForm = "/api/StationByRestaurents/EditDeploy"; //fix
    ...
    getData() {
      gasRetaurantAPI
        .gasRestaurantFinishedProductGet()
        .then((val) => {
  ...
  async created() {
    ...
    await this.getColumn("StationByGasRestaurants");
    await this.getKeyTable();
    await this.getData();
    await this.getFormAddShow("StationByGasRestaurants");

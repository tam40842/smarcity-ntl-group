...
import CustomForm from "./components/GasItemForm";
import handling from "@/constants/handling";
import { timer } from "@/constants/config";
import systemAPI from "@/api/modules/systemAPI";
import gasRetaurantAPI from "@/api/modules/gasRestaurantAPI";
...
export default {
  ...
  methods: {
    ...
    editModal(items) {
      this.getDataByID(items[0].GasItemsID);
      setTimeout(() => {
        ...
        this.typeForm = "EDIT";
        this.apiForm = "/api/GasItems/Edit";
        this.bodyFormDefault = {
          ID: items[0].GasItemsID,
          MenuIDCurent: this.objectData.menuIDCurrent,
        };
        setTimeout(() => {
          this.$bvModal.show(this.nameForm);
    ...
    deleteModal(items) {
      this.$bvModal
        .msgBoxConfirm(this.$t("form.question") + items[0].GasItemsName + "?", {
        ...
        .then((value) => {
          if (value === true) {
            this.handleSubmitDelete(items[0].GasItemsID);
          }
        })
        .catch((err) => {
          console.log(err);
        ...
        .then((val) => {
          if (val.status) {
            if (val.data.length > 0) {
              this.dataFormOptions.ItemGroupID = [];
              for (let i = 0; i < val.data.length; i++) {
                if (val.data[i].IsActive === 1) {
                  let obj = {
                    text: val.data[i].GasItemGroupsName,
                    value: val.data[i].GasItemGroupsID,
                  };
                  this.dataFormOptions.ItemGroupID.push(obj);
                }
              }
            }
          }
    ...
    getOptionGasUnit() {
      gasRetaurantAPI
        ...
        .then((val) => {
          if (val.status) {
            if (val.data.length > 0) {
              this.dataFormOptions.UnitID = [];
              for (let i = 0; i < val.data.length; i++) {
                if (val.data[i].IsActive === 1) {
                  let obj = {
                    text: val.data[i].GasUnitsName,
                    value: val.data[i].GasUnitsID,
                  };
                  this.dataFormOptions.UnitID.push(obj);
                }
              }
            }
          }
  ...
  async created() {
    ...
    await this.getColumn("GasItems");
    await this.getKeyTable();
    await this.getData();
    await this.getFormAddShow("GasItems");
    await this.getOptionGasType();
    await this.getOptionGasUnit();
  },
  watch: {
    items() {
      this.getColumn("GasItems");

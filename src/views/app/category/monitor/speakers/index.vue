...
import CustomTables from "@/views/app/category/monitor/speakers/CustomTables";
...
export default {
  ...
  methods: {
    ...
    addModal() {
      ...
      this.typeForm = "ADD";
      this.apiForm = "/api/StationSpeakers/Add";
    ...
    handleSubmit(val) {
      if (val.status) {
        if (!this.items || this.items.length === 0) {
          ...
          this.getColumn("StationSpeakers");
          this.getKeyTable();
          this.getData();
          setTimeout(() => {
            this.makeToast(
    ...
    editModal(items) {
      ...
      setTimeout(() => {
        ...
        this.typeForm = "EDIT";
        this.apiForm = "/api/StationSpeakers/Edit";
        this.bodyFormDefault = {
          ID: items[0].ID,
          MenuIDCurent: this.objectData.menuIDCurrent,
          UserIDCurent: this.userID,
    ...
    deleteModal(items) {
      this.$bvModal
        .msgBoxConfirm(this.$t("form.question") + items[0].SpeakerName + "?", {
  ...
  async created() {
    ...
    await this.getColumn("StationSpeakers");
    await this.getKeyTable();
    await this.getData();
    await this.getFormAddShow("StationSpeakers");
  },
  watch: {
    items() {
      this.getColumn("StationSpeakers");

...
export default {
    ...
    fields: function () {
      ...
      if (newFields && newFields.length > 0) {
        newFields.forEach((item) => {
          if (
            item.key === "ClearanceHeightTotal" ||
            item.key === "ClearanceHeightAdjust"
          ) {
  ...
  methods: {
    ...
    addModal() {
      ...
      this.typeForm = "ADD";
      this.apiForm = "/api/WaterwayClearanceHeights/Add";
    ...
    handleSubmit(val) {
      if (val.status) {
        if (!this.items || this.items.length === 0) {
          ...
          this.getColumn("WaterwayClearanceHeights");
          this.getKeyTable();
          this.getData();
          setTimeout(() => {
            this.makeToast(
    ...
    editModal(items) {
      ...
      setTimeout(() => {
        ...
        this.typeForm = "EDIT";
        this.apiForm = "/api/WaterwayClearanceHeights/Edit";
        this.bodyFormDefault = {
  ...
  async created() {
    ...
    await this.getColumn("WaterwayClearanceHeights");
    await this.getKeyTable();
    await this.getData();
    await this.getFormAddShow("WaterwayClearanceHeights");
  },
  watch: {
    items() {
      this.getColumn("WaterwayClearanceHeights");

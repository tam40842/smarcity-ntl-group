...
export default {
  ...
  methods: {
    ...
    addModal() {
      ...
      this.typeForm = "ADD";
      this.apiForm = "/api/WaterwaySign/Add";
    ...
    editModal(items) {
      this.getDataByID(items[0].SignID);
      setTimeout(() => {
        ...
        this.typeForm = "EDIT";
        this.apiForm = "/api/WaterwaySign/Edit";
        this.bodyFormDefault = {
          ID: items[0].SignID,
          MenuIDCurent: this.objectData.menuIDCurrent,
          UserIDCurent: this.userID,
        };
        this.dataForm.SideType[1] = handling.convertBitToBoolean(
          this.dataForm.SideType[1]
        );
        setTimeout(() => {
          this.$bvModal.show(this.nameForm);
        }, this.timer);
    ...
    deleteModal(items) {
      this.$bvModal
        .msgBoxConfirm(this.$t("form.question") + items[0].SignName + "?", {
        ...
        .then((value) => {
          if (value === true) {
            this.handleSubmitDelete(items[0].SignID);
          }
        })
        .catch((err) => {
          console.log(err);
    ...
    getOptionWaterwaySignTypeID() {
      waterwayAPI
        .waterWaySignTypeListActive()
        .then((val) => {
          if (val.status) {
            if (val.data.length > 0) {
              this.dataFormOptions.WaterwaySignTypeID = [];
              for (let i = 0; i < val.data.length; i++) {
                let obj = {
                  text: val.data[i].WaterwaySignTypeName,
                  value: val.data[i].ID,
                };
                this.dataFormOptions.WaterwaySignTypeID.push(obj);
              }
            }
          }
        })
  ...
  async created() {
    ...
    await this.getColumn("WaterwaySigns");
    await this.getKeyTable();
    await this.getData();
    await this.getOptionWaterwaySignTypeID();
    await this.getOptionWaterwaySizeID();
    await this.getFormAddShow("WaterwaySigns");
  },
  watch: {
    items() {
      this.getColumn("WaterwaySigns");

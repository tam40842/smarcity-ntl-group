...
export default {
  ...
  methods: {
    ...
    addModal() {
      ...
      this.typeForm = "ADD";
      this.apiForm = "/api/WaterwayBeaconFlashs/Add";
    ...
    handleSubmit(val) {
      if (val.status) {
        if (!this.items || this.items.length === 0) {
          ...
          this.getColumn("WaterwayBeacons");
          this.getKeyTable();
          this.getData();
          setTimeout(() => {
            this.makeToast(
    ...
    editModal(items) {
      ...
      setTimeout(() => {
        ...
        this.typeForm = "EDIT";
        this.apiForm = "/api/WaterwayBeaconFlashs/Edit";
        this.bodyFormDefault = {
          ID: items[0].ID,
          MenuIDCurent: this.objectData.menuIDCurrent,
          UserIDCurent: this.userID,
  ...
  async created() {
    ...
    await this.getColumn("WaterwayBeacons");
    await this.getKeyTable();
    await this.getData();
    await this.getFormAddShow("WaterwayBeacons");
  },
  watch: {
    items() {
      this.getColumn("WaterwayBeacons");

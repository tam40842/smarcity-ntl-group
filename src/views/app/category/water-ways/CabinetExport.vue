...
export default {
  ...
  methods: {
    ...
    updateImageModal(item) {
      this.getImages(item[0].CabinetID);
    ...
    handleSubmitUpdate() {
      let body = {
        UserIDCurent: this.userID,
        MenuIDCurent: this.objectData.menuIDCurrent,
        StationID: this.selectedItems[0].CabinetID,
        GeoCode: this.keyString,
    ...
    addModal() {
      this.apiForm = "/api/WaterwayCabinets/Export";
      setTimeout(() => {
        ...
        .then((val) => {
          if (val.status) {
            if (!this.items || this.items.length === 0) {
              ...
              this.getColumn("SLCabinetExports");
              this.getKeyTable();
              this.getData();
              setTimeout(() => {
                this.makeToast(
    ...
    editModal(items) {
      ...
      setTimeout(() => {
        ...
        this.typeForm = "EDIT";
        this.apiForm = "/api/WaterwayCabinets/Deploy"; //fix
    ...
    deleteModal(items) {
      console.log(items);
      this.$bvModal
        .msgBoxConfirm(this.$t("form.question") + items[0].CabinetName + "?", {
  ...
  async created() {
    ...
    await this.getColumn("SLCabinetExports");
    await this.getKeyTable();
    await this.getData();
    await this.getFormAddShow("SLCabinetExports");
    await this.getConnectTypeOption();
    await this.getPowerTypeOption();
    await this.getWaterwayOption();
  },
  watch: {
    items() {
      this.getColumn("SLCabinetExports");

...
export default {
  ...
  methods: {
    ...
    addModal() {
      ...
      this.typeForm = "ADD";
      this.apiForm = "/api/WaterwayTypes/Add";
    ...
    editModal(items) {
      ...
      setTimeout(() => {
        ...
        this.typeForm = "EDIT";
        this.apiForm = "/api/WaterwayTypes/Edit";
        this.bodyFormDefault = {
          ID: items[0].ID,
          MenuIDCurent: this.objectData.menuIDCurrent,
          UserIDCurent: this.userID,
    ...
    deleteModal(items) {
      this.$bvModal
        .msgBoxConfirm(
          this.$t("form.question") + items[0].WaterwayTypeName + "?",
          {
    ...
    getOptionBeaconFlash() {
      waterwayAPI
        .waterWayBeaconsListActive()
        .then((val) => {
          if (val.status) {
            if (val.data.length > 0) {
              this.dataFormOptions.BeaconFlashID = [];
              for (let i = 0; i < val.data.length; i++) {
                let obj = {
                  text: val.data[i].WaterwayBeaconFlashName,
                  value: val.data[i].ID,
                };
                this.dataFormOptions.BeaconFlashID.push(obj);
              }
            }
          }
        })
    ...
    getOptionBeaconFlashColor() {
      waterwayAPI
        .waterWayBeaconsListColor()
        .then((val) => {
          if (val.status) {
            if (val.data.length > 0) {
              this.dataFormOptions.BeaconColorID = [];
              for (let i = 0; i < val.data.length; i++) {
                let obj = {
                  text: val.data[i].WaterwayBeaconColorName,
                  value: val.data[i].ID,
                };
                this.dataFormOptions.BeaconColorID.push(obj);
              }
            }
          }
        })
  ...
  async created() {
    ...
    await this.getColumn("WaterwayTypes");
    await this.getKeyTable();
    await this.getData();
    await this.getFormAddShow("WaterwayTypes");
    await this.getOptionBeaconFlash();
    await this.getOptionBeaconFlashColor();
  },
  watch: {
    items() {
      this.getColumn("WaterwayTypes");

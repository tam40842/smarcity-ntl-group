...
export default {
  ...
  methods: {
    ...
    addModal() {
      this.apiForm = "/api/WaterwayStations/Export";
      setTimeout(() => {
        ...
        .then((val) => {
          if (val.status) {
            if (!this.items || this.items.length === 0) {
              ...
              this.getColumn("SLStationExports");
              this.getKeyTable();
              this.getData();
              setTimeout(() => {
                this.makeToast(
    ...
    editModal(items) {
      ...
      setTimeout(() => {
        this.dataByID.HasMarker = !!this.dataByID.HasMarker;
        ...
        this.typeForm = "EDIT";
        this.apiForm = "/api/WaterwayStations/Deploy"; //fix
    ...
    deleteModal(items) {
      this.$bvModal
        .msgBoxConfirm(this.$t("form.question") + items[0].StationID + "?", {
  ...
  async created() {
    ...
    await this.getColumn("SLStationExports");
    await this.getKeyTable();
    await this.getData();
    await this.getFormAddShow("SLStationExports");
    await this.getSizeTypeOption();
    await this.getActiveTypeOption();
    await this.getCabinetActiveTypeOption();
  },
  watch: {
    items() {
      this.getColumn("SLStationExports");

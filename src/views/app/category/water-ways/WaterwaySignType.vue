...
export default {
  ...
  methods: {
    ...
    addModal() {
      ...
      this.typeForm = "ADD";
      this.apiForm = "/api/WaterwaySignTypes/Add";
    ...
    handleSubmit(val) {
      if (val.status) {
        if (!this.items || this.items.length === 0) {
          ...
          this.getColumn("WaterwaySignTypes");
          this.getKeyTable();
          this.getData();
          setTimeout(() => {
            this.makeToast(
    ...
    editModal(items) {
      ...
      setTimeout(() => {
        ...
        this.typeForm = "EDIT";
        this.apiForm = "/api/WaterwaySignTypes/Edit";
        this.bodyFormDefault = {
          ID: items[0].ID,
          MenuIDCurent: this.objectData.menuIDCurrent,
          UserIDCurent: this.userID,
    ...
    deleteModal(items) {
      this.$bvModal
        .msgBoxConfirm(
          this.$t("form.question") + items[0].WaterwaySignTypeName + "?",
          {
  ...
  async created() {
    ...
    await this.getColumn("WaterwaySignTypes");
    await this.getKeyTable();
    await this.getData();
    await this.getFormAddShow("WaterwaySignTypes");
  },
  watch: {
    items() {
      this.getColumn("WaterwaySignTypes");

...
export default {
  ...
  methods: {
    ...
    addModal() {
      ...
      this.typeForm = "ADD";
      this.apiForm = "/api/StationByKioskCameras/Add";
    ...
    handleSubmit(val) {
      if (val.status) {
        if (!this.items || this.items.length === 0) {
          ...
          this.getColumn("StationByKioskCameras");
          this.getKeyTable();
          this.getData();
          setTimeout(() => {
            this.makeToast(
    ...
    editModal(items) {
      ...
      setTimeout(() => {
        ...
        this.typeForm = "EDIT";
        this.apiForm = "/api/StationByKioskCameras/Edit";
        this.bodyFormDefault = {
    ...
    deleteModal(items) {
      this.$bvModal
        .msgBoxConfirm(
          this.$t("form.question") + items[0].KioskCameraName + "?",
          {
  ...
  async created() {
    ...
    await this.getColumn("StationByKioskCameras");
    await this.getKeyTable();
    await this.getData();
    await this.getFormAddShow("StationByKioskCameras");
  },
  watch: {
    items() {
      this.getColumn("StationByKioskCameras");

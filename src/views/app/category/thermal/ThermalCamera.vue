...
export default {
  ...
  methods: {
    ...
    addModal() {
      ...
      this.typeForm = "ADD";
      this.apiForm = "/api/StationByThermalCameras/Add";
    ...
    handleSubmit(val) {
      if (val.status) {
        if (!this.items || this.items.length === 0) {
          ...
          this.getColumn("StationByThermalCameras");
          this.getKeyTable();
          this.getData();
          setTimeout(() => {
            this.makeToast(
    ...
    editModal(items) {
      this.getDataByID(items[0].SerialNumber);
      setTimeout(() => {
        ...
        this.typeForm = "EDIT";
        this.apiForm = "/api/StationByThermalCameras/Edit";
        this.bodyFormDefault = {
    ...
    deleteModal(items) {
      this.$bvModal
        .msgBoxConfirm(
          this.$t("form.question") + items[0].ThermalCameraName + "?",
          {
        ...
        .then((value) => {
          if (value === true) {
            this.handleSubmitDelete(items[0].SerialNumber);
          }
        })
        .catch((err) => {
          console.log(err);
  ...
  async created() {
    ...
    await this.getColumn("StationByThermalCameras");
    await this.getKeyTable();
    await this.getData();
    await this.getFormAddShow("StationByThermalCameras");
  },
  watch: {
    items() {
      this.getColumn("StationByThermalCameras");

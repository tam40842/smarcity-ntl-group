...
export default {
  ...
  methods: {
    ...
    addModal() {
      ...
      this.typeForm = "ADD";
      this.apiForm = "/api/LightingMaintenanceErrors/Add";
    ...
    editModal(items) {
      this.getDataByID(items[0].MaintenanceErrorID);
      setTimeout(() => {
        ...
        this.typeForm = "EDIT";
        this.apiForm = "/api/LightingMaintenanceErrors/Edit";
        this.bodyFormDefault = {
          ID: items[0].MaintenanceErrorID,
          MenuIDCurent: this.objectData.menuIDCurrent,
        };
        setTimeout(() => {
          this.$bvModal.show(this.nameForm);
    ...
    deleteModal(items) {
      this.$bvModal
        .msgBoxConfirm(
          this.$t("form.question") + items[0].MaintenanceErrorID + "?",
          {
        ...
        .then((value) => {
          if (value === true) {
            this.handleSubmitDelete(items[0].MaintenanceErrorID);
          }
        })
        .catch((err) => {
          console.log(err);
  ...
  async created() {
    ...
    await this.getColumn("MaintenanceErrors");
    await this.getKeyTable();
    await this.getData();
    await this.getFormAddShow("MaintenanceErrors");
  },
  watch: {
    items() {
      this.getColumn("MaintenanceErrors");
      this.getKeyTable();
    },
  },
  computed: {

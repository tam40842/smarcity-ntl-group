        ...
        <div class="top-right-button-container">
          <b-button-group>
            <b-dropdown right :text="$t('dropdown.action')" variant="primary">
              ...
              <!-- <b-dropdown-item @click="printExternalExcel('custom-table')"
                               v-if="items && items.length > 0 && objectData.accessWrite === true">
                <i class="fad fa-file-excel"></i>&emsp; {{ $t('report.action.download-excel-page') }}
              </b-dropdown-item>
              <b-dropdown-item v-else disabled>
                <i class="fad fa-file-excel"></i>&emsp; {{ $t('report.action.download-excel-page') }}
              </b-dropdown-item> -->
...
export default {
  ...
  methods: {
    ...
    addModal() {
      ...
      this.typeForm = "ADD";
      this.apiForm = "/api/Positions/Add";
    ...
    editModal(items) {
      this.getDataByID(items[0].PositionID);
      setTimeout(() => {
        ...
        this.typeForm = "EDIT";
        this.apiForm = "/api/Positions/Edit";
        this.bodyFormDefault = {
          ID: items[0].PositionID,
          MenuIDCurent: this.objectData.menuIDCurrent,
        };
        setTimeout(() => {
          this.$bvModal.show(this.nameForm);
    ...
    deleteModal(items) {
      this.$bvModal
        .msgBoxConfirm(this.$t("form.question") + items[0].PositionName + "?", {
        ...
        .then((value) => {
          if (value === true) {
            this.handleSubmitDelete(items[0].PositionID);
          }
        })
        .catch((err) => {
          console.log(err);
  ...
  async created() {
    ...
    await this.getColumn("Positions");
    await this.getKeyTable();
    await this.getData();
    await this.getFormAddShow("Positions");
  },
  watch: {
    items() {
      this.getColumn("Positions");

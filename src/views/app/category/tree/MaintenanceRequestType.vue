...
export default {
  ...
  methods: {
    ...
    addModal() {
      ...
      this.typeForm = "ADD";
      this.apiForm = "/api/MaintenanceRequestType/Add";
    ...
    handleSubmit(val) {
      if (val.status) {
        if (!this.items || this.items.length === 0) {
          ...
          this.getColumn("MaintenanceRequestType");
          this.getKeyTable();
          this.getData();
          setTimeout(() => {
            this.makeToast(
    ...
    editModal(items) {
      this.getDataByID(items[0].MaintenanceRequestTypeID);
      setTimeout(() => {
        ...
        this.typeForm = "EDIT";
        this.apiForm = "/api/MaintenanceRequestType/Edit";
        this.bodyFormDefault = {
          ID: items[0].MaintenanceRequestTypeID,
          MenuIDCurent: this.objectData.menuIDCurrent,
        };
        setTimeout(() => {
          this.$bvModal.show(this.nameForm);
    ...
    deleteModal(items) {
      this.$bvModal
        .msgBoxConfirm(
          this.$t("form.question") + items[0].MaintenanceRequestTypeName + "?",
          {
        ...
        .then((value) => {
          if (value === true) {
            this.handleSubmitDelete(items[0].MaintenanceRequestTypeID);
          }
        })
        .catch((err) => {
          console.log(err);
    ...
    statusEdit(data) {
      let body = {
        ID: data.MaintenanceRequestTypeID,
  ...
  async created() {
    ...
    await this.getColumn("MaintenanceRequestType");
    await this.getKeyTable();
    await this.getData();
    await this.getFormAddShow("MaintenanceRequestType");
  },
  watch: {
    items() {
      this.getColumn("MaintenanceRequestType");

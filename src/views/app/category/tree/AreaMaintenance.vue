...
export default {
  ...
  methods: {
    ...
    addModal() {
      ...
      this.typeForm = "ADD";
      this.apiForm = "/api/MaintenanceArea/Add";
    ...
    handleSubmit(val) {
      if (val.status) {
        if (!this.items || this.items.length === 0) {
          ...
          this.getColumn("MaintenanceArea");
          this.getKeyTable();
          this.getData();
          setTimeout(() => {
            this.makeToast(
    ...
    editModal(items) {
      this.getDataByID(items[0].MaintenanceAreaID);
      setTimeout(() => {
        ...
        this.typeForm = "EDIT";
        this.apiForm = "/api/MaintenanceArea/Edit";
        this.bodyFormDefault = {
          ID: items[0].MaintenanceAreaID,
          MenuIDCurent: this.objectData.menuIDCurrent,
        };
        setTimeout(() => {
          this.$bvModal.show(this.nameForm);
    ...
    deleteModal(items) {
      this.$bvModal
        .msgBoxConfirm(
          this.$t("form.question") + items[0].MaintenanceAreaName + "?",
          {
        ...
        .then((value) => {
          if (value === true) {
            this.handleSubmitDelete(items[0].MaintenanceAreaID);
          }
        })
        .catch((err) => {
          console.log(err);
  ...
  async created() {
    ...
    await this.getColumn("MaintenanceArea");
    await this.getKeyTable();
    await this.getData();
    await this.getFormAddShow("MaintenanceArea");
  },
  watch: {
    items() {
      this.getColumn("MaintenanceArea");

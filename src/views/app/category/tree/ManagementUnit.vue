...
export default {
  ...
  methods: {
    ...
    addModal() {
      ...
      this.typeForm = "ADD";
      this.apiForm = "/api/ManagementUnit/Add";
    ...
    editModal(items) {
      this.getDataByID(items[0].ManagementUnitID);
      setTimeout(() => {
        ...
        this.typeForm = "EDIT";
        this.apiForm = "/api/ManagementUnit/Edit";
        this.bodyFormDefault = {
          ID: items[0].ManagementUnitID,
          MenuIDCurent: this.objectData.menuIDCurrent,
        };
        setTimeout(() => {
          this.$bvModal.show(this.nameForm);
    ...
    deleteModal(items) {
      this.$bvModal
        .msgBoxConfirm(
          this.$t("form.question") + items[0].ManagementUnitName + "?",
          {
        ...
        .then((value) => {
          if (value === true) {
            this.handleSubmitDelete(items[0].ManagementUnitID);
          }
        })
        .catch((err) => {
          console.log(err);
  ...
  async created() {
    ...
    await this.getColumn("ManagementUnit");
    await this.getKeyTable();
    await this.getData();
    await this.getFormAddShow("ManagementUnit");
  },
  watch: {
    items() {
      this.getColumn("ManagementUnit");

...
export default {
  ...
  methods: {
    ...
    printExternalExcel(table) {
      handling.printExcel(
        table,
        this.dataParent.FireLocationName.toUpperCase(),
        null,
        null,
        ...
        .then((val) => {
          if (val.status) {
            if (!this.items || this.items.length === 0) {
              ...
              this.getColumn("FireLocationDetails");
    ...
    editModal(items) {
      ...
      setTimeout(() => {
        ...
        this.nameForm = "form-location-detail";
        this.titleForm = "CẬP NHẬT TRIỄN KHAI";
        this.typeForm = "EDIT";
        this.apiForm = "/api/FireLocationDetails/Edit";
        this.bodyFormDefault = {
          ID: items[0].ID,
          MenuIDCurent: this.objectData.menuIDCurrent,
        };
    ...
    deleteModal(items) {
      console.log(items);
      this.$bvModal
        .msgBoxConfirm(this.$t("form.question") + items[0].FireLocationDetailName + "?", {
    ...
    statusEdit(data, field) {
      ...
      fireAPI
        .fireLocationDetailChangeStatus(body)
        .then((val) => {
          if (val.status) {
            setTimeout(() => {
              this.handleReload(this.dataParent);
  ...
  async created() {
    await this.getFormAddShow("FireLocationDetails");

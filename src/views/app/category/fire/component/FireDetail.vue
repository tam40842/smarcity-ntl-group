<template>
  <div v-if="objectData">
    <b-row>
      <b-colxx xxs="12">
        <h1>
          <strong>{{ data.FireLocationName.toUpperCase() }}</strong>
        </h1>
        <div class="top-right-button-container">
          <b-button-group>
            <b-dropdown right :text="$t('dropdown.action')" variant="primary">
              ...
              <b-dropdown-item
                @click="printExternalExcel('custom-table-report-fire-detail')"
                v-if="
    ...
    <b-row>
      <b-colxx xxs="12" class="mb-4">
        <custom-table
          ref="tableForm"
          :id="'custom-table-report-fire-detail'"
          :data-table="items"
          :field-table="fields"
...
import {
  ...
  fireDetailGet,
  fireDetailByID,
  fireDetailChangeStatus,
  fireDetailRemove,
} from "@/data/modules/fire";
...
export default {
  ...
  methods: {
    ...
    addModal() {
      ...
      this.nameForm = "modal-add-fire-detail";
      this.titleForm = this.$t("form.title-add").toUpperCase();
      this.typeForm = "ADD";
      this.apiForm = "/api/FireDetails/Add";
    ...
    editModal(items) {
      ...
      setTimeout(() => {
        ...
        this.nameForm = "modal-edit-fire-detail";
        this.titleForm = this.$t("form.title-edit").toUpperCase();
        this.typeForm = "EDIT";
        this.apiForm = "/api/FireDetails/Edit";
        this.bodyFormDefault = {
          ID: items[0].ID,
          MenuIDCurent: this.objectData.menuIDCurrent,
        };
    ...
    deleteModal(items) {
      this.$bvModal
        .msgBoxConfirm(this.$t("form.question") + items[0].Code + "?", {
  ...
  async created() {
    ...
    await this.getColumn("FireDetails");
    await this.getKeyTable();
    await this.getData();
    await this.getFormAddShow("FireDetails");
    await this.getStationOption(this.data.ID);
  },
  watch: {
    items() {
      this.getColumn("FireDetails");

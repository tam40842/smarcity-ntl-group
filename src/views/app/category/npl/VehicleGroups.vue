  ...
  methods: {
          ...
          if (val.status && val.data?.length > 0) {
            ...
            val.data.forEach((item) => {
              if (item.IsActive === 1) {
                ...
                this.dataFormOptions.UserID.push(obj);
              }
            });
          }
        })
    ...
    addModal() {
      ...
      this.dataForm.forEach((f) => {
        if (f.name == "UserID") {
          f.value = this.dataFormOptions.UserID[0].value;
        }
      });
      this.idModal = "add";

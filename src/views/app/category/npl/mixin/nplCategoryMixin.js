...
export default {
  ...
  data() {
    return {
      ...
      dataSearch: [],
      isSearching: false,
      notRequired: ["ID", "Note", "NameExtention1", "AddressExtention1", "EntryID"],
      formDiffList: ['nPLDrivers', 'nPLDriveHelpers', 'LeaveReasons', 'VehicleGroups', 'Vehicles', 'SMRMs', 'FuelKinds', 'OutsourcedVehicles', 'OutsourcedVehicleProviders', 'nPLCustomers', 'Banks', 'GoodsTypes', 'nplRegions', 'InspectionUnits', 'ChargingStations', 'Ports', 'PetrolStations', 'nPLStations', 'MaintenanceJobs', 'MaintenanceSupplies', 'nPLRoutes'],
  ...
  computed: {
    ...
    fieldsTable() {
      if (this.objectKeys.length > 0 && this.colTypes.length > 0) {
        const result = handling.mergeKeyDynamic(this.objectKeys, this.colTypes);
        result.forEach(item => {
          item.sortable = false
  ...
  methods: {
    ...
    getSearchOptionByType(type, arr) {
      let option = [...arr];
      if (type.toUpperCase() === "STRING" || type.toUpperCase() === "EMAIL" || type.toUpperCase() === "NOTE") {
        option = arr.filter(
          (item) =>
            item.value === "=" ||
            item.value === "like" ||

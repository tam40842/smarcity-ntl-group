    ...
    <template v-if="dataForm">
      <b-col lg="12" class="pl-0">
        ...
        <b-row>
          ...
          <b-col lg="2">
            <b-form-group
              :label="dataForm.MaintenanceSupplyType[0]"
              :invalid-feedback="$t('form.no-blank')"
              :state="dataForm.MaintenanceSupplyType[3]"
            >
              <b-form-input
                v-model="dataForm.MaintenanceSupplyType[1]"
                :disabled="
                  currentMode === 'adding' || currentMode === 'editing'
                    ? false
                    : true
          ...
          <b-col lg="2">
            <b-form-group
              :label="dataForm.MaintenanceSupplyProducer[0]"
              :invalid-feedback="$t('form.no-blank')"
              :state="dataForm.MaintenanceSupplyProducer[3]"
            >
              <b-form-select
                v-model="dataForm.MaintenanceSupplyProducer[1]"
                :options="dataFormOptions.MaintenanceSupplyProducer"
                :disabled="
                  currentMode === 'adding' || currentMode === 'editing'
                    ? false
                    : true
        ...
        <b-row>
          ...
          <b-col lg="2">
            <b-form-group
              :label="dataForm.UnitID[0]"
              :invalid-feedback="$t('form.no-blank')"
              :state="dataForm.UnitID[3]"
            >
              <b-form-select
                v-model="dataForm.UnitID[1]"
                :options="dataFormOptions.UnitID"
                :disabled="
                  currentMode === 'adding' || currentMode === 'editing'
                    ? false
                    : true
...
export default {
  ...
  created() {
    this.getSelect("nPLUnits", "nPLUnitsName", "UnitID");
    this.getSelect("VehicleProducers", "VehicleProducersName", "MaintenanceSupplyProducer");
  },
};
</script>

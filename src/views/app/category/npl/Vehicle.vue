...
export default {
  ...
  methods: {
    ...
    getVehicleTypes() {
      let body = {
        CategoryType: "VehicleTypes",
      };
      nplAPI
        .getCategoryActive(body)
        .then((val) => {
          if (val.status && val.data?.length > 0) {
            this.dataFormOptions.VehicleTypeID = [];
            val.data.forEach((item) => {
              if (item.IsActive === 1) {
                let obj = {
                  value: item.ID,
                  text: item.VehicleTypesName,
                  note: item.Note,
                };
                this.dataFormOptions.VehicleTypeID.push(obj);
              }
            });
          }
        })
    ...
    addModal() {
      ...
      this.dataForm.forEach((f) => {
        switch (f.name) {
          case "VehicleTypeID":
            f.value = this.dataFormOptions.VehicleTypeID[0].value;
            break;
          case "VehicleTyreID":
            f.value = this.dataFormOptions.VehicleTyreID[0].value;
            break;
          case "VehicleProducerID":
            f.value = this.dataFormOptions.VehicleProducerID[0].value;
            break;
          case "VehicleColorID":
            f.value = this.dataFormOptions.VehicleColorID[0].value;
            break;
          case "DriveHelperID":
            f.value = this.dataFormOptions.DriveHelperID[0].value;
            break;
          case "FuelKindID":
            f.value = this.dataFormOptions.FuelKindID[0].value;
            break;
          default:
            break;
        }

...
import FormRoute from "./component/FormRoute.vue";
  ...
  methods: {
    handleSubmit(dataValue, type, id) {
      ...
      if (type == "add") {
        ...
        nplAPI.routesDelete(body).then((val) => {
          if (val.status) {
            this.makeToast("success", this.$t("toast.success"), val.message);
            this.getData();
    ...
    handleDbClick(row) {
      ...
      nplAPI.routesByID(body).then((val) => {
        if (val.status) {

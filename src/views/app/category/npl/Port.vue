  ...
  methods: {
    handleSubmit(dataValue, type, id) {
      ...
      if (type == "add") {
        ...
        dataValue.forEach((f) => {
          let key = f["name"];
          if (key == "Container20Cost" || key == "Container40Cost") {
    ...
    addModal() {
      ...
      this.dataForm.forEach((f) => {
        if (f.name == "BankID") {
          f.value = this.dataFormOptions.BankID[0].value;
        }
      });
      this.idModal = "add";
      ...
      nplAPI
        ...
        .then((val) => {
          if (val.status && val.data?.length > 0) {
            this.dataFormOptions.BankID = [];
            for (let i = 0; i < val.data.length; i++) {
              if (val.data[i].IsActive === 1) {
                let obj = {
                  text: val.data[i].BanksName,
                  value: val.data[i].ID,
                };
                this.dataFormOptions.BankID.push(obj);
              }
            }
          }
        })

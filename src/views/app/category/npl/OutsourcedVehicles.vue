...
export default {
  ...
  methods: {
    getOption() {
      let body = {
        CategoryType: "OutsourcedVehicleProviders",
      };
      nplAPI
        .getCategoryActive(body)
        .then((val) => {
          if (val.status && val.data?.length > 0) {
            this.dataFormOptions.OVPID = [];
            val.data.forEach((item) => {
              if (item.IsActive === 1) {
                let obj = {
                  value: item.ID,
                  text: item.OutsourcedVehicleProvidersName,
                };
                this.dataFormOptions.OVPID.push(obj);
              }
            });
          }
        })
    ...
    addModal() {
      ...
      this.dataForm.forEach((f) => {
        if (f.name == "OVPID") {
          f.value = this.dataFormOptions.OVPID[0].value;
        }
      });
      this.idModal = "add";

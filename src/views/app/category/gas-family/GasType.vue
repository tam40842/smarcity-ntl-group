...
export default {
  ...
  methods: {
    ...
    editModal(items) {
      this.getDataByID(items[0].GastypeID);
      setTimeout(() => {
        ...
        this.typeForm = "EDIT";
        this.apiForm = "/api/GasTypes/Edit";
        this.bodyFormDefault = {
          ID: items[0].GastypeID,
          MenuIDCurent: this.objectData.menuIDCurrent,
        };
        setTimeout(() => {
          this.$bvModal.show(this.nameForm);
    ...
    deleteModal(items) {
      this.$bvModal
        .msgBoxConfirm(this.$t("form.question") + items[0].GastypeName + "?", {
        ...
        .then((value) => {
          if (value === true) {
            this.handleSubmitDelete(items[0].GastypeID);
          }
        })
        .catch((err) => {
          console.log(err);
  ...
  async created() {
    ...
    await this.getColumn("GasTypes");
    await this.getKeyTable();
    await this.getData();
    await this.getFormAddShow("GasTypes");
  },
  watch: {
    items() {
      this.getColumn("GasTypes");

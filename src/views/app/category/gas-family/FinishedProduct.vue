...
export default {
  ...
  methods: {
    ...
    addModal() {
      this.apiForm = "/api/StationByGasFamilys/ExportProduct";
      setTimeout(() => {
        ...
        .then((val) => {
          if (val.status) {
            if (!this.items || this.items.length === 0) {
              ...
              this.getColumn("StationByGasFamilys");
              this.getKeyTable();
              this.getData();
              setTimeout(() => {
                this.makeToast(
    ...
    editModal(items) {
      ...
      setTimeout(() => {
        ...
        this.typeForm = "EDIT";
        this.apiForm = "/api/StationByGasFamilys/EditDeploy";
        ...
        .then((val) => {
          if (val.status) {
            if (val.data.length > 0) {
              this.dataFormOptions.GasTypeID = [];
              for (let i = 0; i < val.data.length; i++) {
                if (val.data[i].IsActive === 1) {
                  let obj = {
                    text: val.data[i].GastypeName,
                    value: val.data[i].GastypeID,
                  };
                  this.dataFormOptions.GasTypeID.push(obj);
                }
              }
            }
          }
  ...
  async created() {
    ...
    await this.getColumn("StationByGasFamilys");
    await this.getKeyTable();
    await this.getData();
    await this.getFormAddShow("StationByGasFamilys");

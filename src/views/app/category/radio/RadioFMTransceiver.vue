  ...
  methods: {
    handleSubmit(dataValue, type, id) {
      ...
      if (type == 'add') {
        ...
        radioAPI.radioFMDelete(body).then((val) => {
          if (val.status) {
            this.makeToast('success', this.$t('toast.success'), val.message)
            this.getData()
    ...
    changeStatus(row, key) {
      const id = row.FMTransceiverID
      const newStatus = handling.convertBooleanToBit(row.IsActive)
      let body = {
        ID: id,
        IsActive: newStatus,
      }
      radioAPI.radioFMChangeStatus(body)
        .then((val) => {
          if (val.status) {
            this.makeToast('success', this.$t('toast.success'), val.message)
          } else {
    ...
    handleDbClick(row) {
      this.idClick = row.FMTransceiverID
      let body = {
        ID: row.FMTransceiverID,
      }
      radioAPI.radioFMByID(body).then((val) => {
        if (val.status) {

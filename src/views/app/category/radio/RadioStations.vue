...
export default {
  ...
  methods: {
    ...
    handleSubmitUpdate() {
      ...
      this.listImageBase64.forEach((item) => {
        let link = item.Images.split(',')
        if (link.length > 1) {
          body.Base64Strings.push(link[1])
        } else {
          body.Base64Strings.push(link[0])
        }
      })
      systemAPI.uploadImagesBase64(body)
        .then((val) => {
          if (val.status) {
    ...
    getOptionDataTypeID(code) {
      ...
      systemAPI.getDataTypeByID(body)
        .then((val) => {
          if (val.status) {
    ...
    editModal(items) {
      ...
      setTimeout(() => {
        ...
        this.typeForm = 'EDIT'
        this.apiForm = '/api/RadioStations/Deploy' //fix
    ...
    statusEdit(data) {
      ...
      radioAPI.radioChangeStatus(body)
        .then((val) => {
          if (val.status) {
            setTimeout(() => {
              this.getData()
    ...
    getOptionConnectType() {
      systemAPI.shareConnectTypeList()
        .then((val) => {
          if (val.status) {
            if (val.data.length > 0) {
  ...
  async created() {
    ...
    await this.getColumn('RadioStations')
    await this.getKeyTable()
    await this.getData()
    await this.getFormAddShow('RadioStations')

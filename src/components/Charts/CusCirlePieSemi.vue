<template>
  <div>
    <div class="pie-wrapper">
      <div class="arc" :data-value="value"></div>
      <span class="score" :style="value < 0 ? 'color:red' : null">
        {{ value < 0 ? $t("map.error-sensor") : label }}
      </span>
    </div>
  </div>
</template>
...
export default {
  data() {
    return {
      _width: this.width ?? "160px",
      _height: this.width / 2 ?? "80px",
    };
  },
  mounted() {
    setTimeout(() => {
      if (this.value > 100 && !this.rankValue) {
        this.value = 100;
      }
      if (this.rankValue) {
        this.value = (this.value / this.rankValue) * 100;
      }
    }, 500);
  },
  props: ["value", "label", "rankValue", "width"],
};
</script>
<style scoped>
.pie-wrapper {

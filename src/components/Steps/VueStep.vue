<template>
  <div :class="[`v-step-warp-${direction}`, styleType]">
    <div class="v-step-progress-bg">
      <div class="v-step-progress-value" :style="progressStyle" />
    </div>
    <ul class="v-step-list">
      <li
        v-for="(obj, index) in stepList"
        :key="index"
        class="v-step-item"
      ...
      >
        <label class="v-step-item-label">
          <div style="font-weight:650">{{ obj.label }}</div>
          <div>{{obj.name ? obj.name : 'Chưa cập nhật'}} - <span class="text-primary">{{convertDatetime(obj.datetime)}}</span></div>
        </label>
        <div class="v-step-item-number" :style="itemNumberStyle(index)">

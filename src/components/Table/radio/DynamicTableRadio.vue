  ...
  <div class="dynamic-table">
    ...
    >
      ...
      >
        ...
        <template
          v-else-if="field.typeCol.toUpperCase() == 'ACTIONS_VOCALSTRENGTH'"
        >
          <div :key="index">
            <b-form-select
              ...
              size="sm"
              @change="emitChangeVolcal(row.item, row.item[`${field.key}`])"
            ></b-form-select>
          </div>
        </template>
        <template
          v-else-if="field.typeCol.toUpperCase() == 'ACTIONS_PLAYBACKSTATE'"
        >
          <div :key="index" class="d-flex">
            <b-button
              @click.prevent="emitPlayback(row.item, 'OFF')"
              ...
              class="mr-1 d-flex"
              variant="danger"
              ><i class="far fa-stop-circle mr-1"></i>Stop</b-button
            >
            <b-button
              @click.prevent="emitPlayback(row.item, 'PAUSE')"
              style="font-weight: 600"
              v-if="row.item[`${field.key}`] == 1"
              size="sm"
              squared
            ...
            <b-button
              @click.prevent="emitPlayback(row.item, 'ON')"
              style="font-weight: 600"
              v-if="
                row.item[`${field.key}`] == 0 || row.item[`${field.key}`] == 2
              "
              size="sm"
              squared
              class="mr-1 d-flex"
              ...
              {{
                row.item[`${field.key}`] == 0 ? "Play" : "Continue"
              }}</b-button
            >
          </div>
        </template>
...
export default {
  ...
  data() {
    return {
      ...
      currentPage: 1,
      perPage: this.pPerPage,
      //vocal
      vocalOptions: [
        { text: "Mức 10%", value: 10 },
        { text: "Mức 20%", value: 20 },
        { text: "Mức 30%", value: 30 },
        { text: "Mức 40%", value: 40 },
        { text: "Mức 50%", value: 50 },
        { text: "Mức 60%", value: 60 },
        { text: "Mức 70%", value: 70 },
        { text: "Mức 80%", value: 80 },
        { text: "Mức 90%", value: 90 },
        { text: "Mức 100%", value: 100 },
      ],
    };
  },
  async created() {
  ...
  methods: {
    emitChangeVolcal(item, value) {
      this.$emit("change-vocal", item, value);
    },
    emitPlayback(item, action) {
      this.$emit("change-playback", item, action);
    },
    loadImgDefault(e) {
      return (e.target.src =

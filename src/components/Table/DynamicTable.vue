      ...
      <b-col :lg="7">
        <div class="pt-3">
          ...
          <span v-else>
            ...
            {{ $t("other.result") }} (Trang <b>{{ pCurrentPage }} </b>)
          </span>
        </div>
      </b-col>
      <b-col :lg="5">
          ...
          <div :key="index" class="p-0 position-relative">
            ...
            <!-- <template v-else>
              <audio-player
                v-if="
                  row.item[`${field.key}`] &&
                  row.item[`${field.key}`].length > 0
                "
                class="audioPlayerTableDynamic"
                ref="audioPlayerTableDynamic"
                :audio-list="row.item[`${field.key}`].split(';')"
